            ...
            <x-slot:body>
                @forelse ($sertifikats ?? [] as $sertifikat)
                    ...
                    <tr x-show="search === '' || '{{ $searchable }}'.includes(search.toLowerCase())">
                        <td x-html="highlight('{{ addslashes($sertifikat->title) }}')"></td>
                        <td width="1%">
                            <div class="table__action">
                                <x-button type="link" href="{{ route('dashboard.sertifikat.download', compact('sertifikat')) }}" class="button__info"><i class="fa-solid fa-download"></i>Download</x-button>
                            </div>
                        </td>
                    </tr>
                @empty
