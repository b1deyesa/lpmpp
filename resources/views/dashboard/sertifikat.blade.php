<x-layout.dashboard class="sertifikat">
    
    {{-- Title --}}
    <h1 class="dashboard__title">Sertifikat</h1>
    ...
    <div class="dashboard__file" x-data="{ search: '', highlight(text) { if (!this.search) return text; const keyword = this.search.toLowerCase(); const source  = text.toLowerCase(); let result = ''; let i = 0; while (i < text.length) { const index = source.indexOf(keyword, i); if (index === -1) { result += text.slice(i); break; } result += text.slice(i, index); result += `<mark>${text.slice(index, index + this.search.length)}</mark>`; i = index + this.search.length; } return result; } }">
        ...
        <div class="file__features">
            @livewire('dashboard.sertifikat-category.create')
            <div class="features__right">
                ...
                <x-modal>
                    ...
                    <x-form action="{{ route('dashboard.sertifikat.truncate') }}" method="POST">
                        <x-slot:bottom>
        ...
        {{-- List --}}
        @if (count($sertifikat_categories) > 0 || count($sertifikats) > 0)
                    ...
                    @foreach ($sertifikat_categories as $sertifikat_category)
                        @php
                            $searchable = strtolower(
                                implode(' ', [
                                    $sertifikat_category->title
                                ])
                            );
                        @endphp
                        <a href="{{ route('dashboard.sertifikat-category.show', compact('sertifikat_category')) }}" class="list__item" x-show="search === '' || '{{ $searchable }}'.includes(search.toLowerCase())">
                            <div class="item__left">
                                <i class="item__icon fa-regular fa-folder-open"></i>
                                <h3 class="item__title" x-html="highlight('{{ addslashes($sertifikat_category->title) }}')"></h3>
                            </div>
                        </a>
                    @endforeach
                </section>
