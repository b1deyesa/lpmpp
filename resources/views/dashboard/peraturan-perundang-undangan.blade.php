<x-layout.dashboard class="peraturan-perundang-undangan">
    ...
    <div class="dashboard__file" x-data="{ search: '', highlight(text) { if (!this.search) return text; const keyword = this.search.toLowerCase(); const source  = text.toLowerCase(); let result = ''; let i = 0; while (i < text.length) { const index = source.indexOf(keyword, i); if (index === -1) { result += text.slice(i); break; } result += text.slice(i, index); result += `<mark>${text.slice(index, index + this.search.length)}</mark>`; i = index + this.search.length; } return result; } }">
        ...
        <div class="file__features">
            @livewire('dashboard.peraturan-perundang-undangan-category.create')
        ...
        {{-- List --}}
        @if (count($peraturan_perundang_undangan_categories) > 0 || count($peraturan_perundang_undangans) > 0)
            <div class="file__list">
                
                {{-- Category --}}
                <section>
                    @foreach ($peraturan_perundang_undangan_categories as $pp_category)
                        @php
                            $searchable = strtolower($pp_category->title);
                        @endphp
                        <a href="{{ route('dashboard.peraturan-perundang-undangan-category.show', ['ppuCategory' => $pp_category]) }}" class="list__item" x-show="search === '' || '{{ $searchable }}'.includes(search.toLowerCase())">
                            <div class="item__left">
                                <i class="item__icon fa-regular fa-folder-open"></i>
                                <h3 class="item__title" x-html="highlight('{{ addslashes($pp_category->title) }}')"></h3>
                            </div>
                        </a>
                    @endforeach
                </section>
