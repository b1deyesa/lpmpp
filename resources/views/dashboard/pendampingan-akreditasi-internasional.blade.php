<x-layout.dashboard class="pendampingan-akreditasi-internasional">
    ...
    {{-- Form --}}
    <x-form action="{{ route('dashboard.pendampingan-akreditasi-internasional.store') }}" method="POST">
        <x-input type="editor" name="body" toolbar="full" value="{{ $pendampingan_akreditasi_internasional?->body }}" />
        <x-slot:bottom>
            <x-modal>
                <x-slot:trigger>
