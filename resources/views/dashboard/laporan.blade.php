            ...
            <x-slot:body>
                @forelse ($laporans ?? [] as $laporan)
                    ...
                    <tr x-show="search === '' || '{{ $searchable }}'.includes(search.toLowerCase())">
                        <td x-html="highlight('{{ addslashes($laporan->title) }}')"></td>
                        <td width="1%">
                            <div class="table__action">
                                <x-button type="link" href="{{ route('dashboard.laporan.download', compact('laporan')) }}" class="button__info"><i class="fa-solid fa-download"></i>Download</x-button>
                            </div>
                        </td>
                    </tr>
                @empty
