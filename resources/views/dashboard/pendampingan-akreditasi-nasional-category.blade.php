<x-layout.dashboard class="pendampingan-akreditasi-nasional-category">
    
    {{-- Title --}}
    <h1 class="dashboard__title">Pendampingan Akreditasi Nasional ({{ $pendampingan_akreditasi_nasional_category->title }})</h1>
    ...
    <div class="dashboard__file" x-data="{ search: '', highlight(text) { if (!this.search) return text; const keyword = this.search.toLowerCase(); const source  = text.toLowerCase(); let result = ''; let i = 0; while (i < text.length) { const index = source.indexOf(keyword, i); if (index === -1) { result += text.slice(i); break; } result += text.slice(i, index); result += `<mark>${text.slice(index, index + this.search.length)}</mark>`; i = index + this.search.length; } return result; } }">
        ...
        <div class="file__features">
            <div class="features__right">
                ...
                <x-modal>
                    ...
                    <x-form action="{{ route('dashboard.pendampingan-akreditasi-nasional-category.truncate', ['panCategory' => $pendampingan_akreditasi_nasional_category]) }}" method="POST">
                        <x-slot:bottom>
        ...
        {{-- List --}}
        @if (count($pendampingan_akreditasi_nasional_category->pendampinganAkreditasiNasionals) > 0)
