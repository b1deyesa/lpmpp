<x-layout.dashboard class="pendampingan-akreditasi-nasional">
    ...
    {{-- Form --}}
    <x-form action="{{ route('dashboard.pendampingan-akreditasi-nasional.store') }}" method="POST">
        <x-input type="editor" name="body" toolbar="full" value="{{ $pendampingan_akreditasi_nasional?->body }}" />
        <x-slot:bottom>
            <x-modal>
                <x-slot:trigger>
