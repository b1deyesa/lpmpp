<x-layout.dashboard class="layanan-bkd">
    ...
    <div class="dashboard__file" x-data="{ search: '', highlight(text) { if (!this.search) return text; const keyword = this.search.toLowerCase(); const source  = text.toLowerCase(); let result = ''; let i = 0; while (i < text.length) { const index = source.indexOf(keyword, i); if (index === -1) { result += text.slice(i); break; } result += text.slice(i, index); result += `<mark>${text.slice(index, index + this.search.length)}</mark>`; i = index + this.search.length; } return result; } }">
        ...
        <div class="file__features">
            @livewire('dashboard.layanan-bkd-category.create')
        ...
        {{-- List --}}
        @if (count($layanan_bkd_categories) > 0 || count($layanan_bkds) > 0)
                    ...
                    @foreach ($layanan_bkd_categories as $layanan_bkd_category)
                        @php
                            $searchable = strtolower(
                                implode(' ', [
                                    $layanan_bkd_category->title
                                ])
                            );
                        @endphp
                        <a href="{{ route('dashboard.layanan-bkd-category.show', compact('layanan_bkd_category')) }}" class="list__item" x-show="search === '' || '{{ $searchable }}'.includes(search.toLowerCase())">
                            <div class="item__left">
                                <i class="item__icon fa-regular fa-folder-open"></i>
                                <h3 class="item__title" x-html="highlight('{{ addslashes($layanan_bkd_category->title) }}')"></h3>
                            </div>
                        </a>
                    @endforeach
                </section>
